...
- **`regression/`** — the honest null, locked: `pure_noise` -> non-significant
  Memmel-JK and `DSR <= 0`; the `k_blocks` recovery guard (ARI vs truth >=
  threshold); the **RMT denoise-on/off ablation** table reported honestly; the DSR
  trial-count guard; `to_dict()` snapshots; valid-Plotly-JSON / explicit-`null`
  API-shape checks.
